...
//void draw_grid();
void draw_floor();
void print( float px, float vx, float dx, unsigned char yy );
...
void main( void )
{
	...
	for( ;;)
	{
		...
		if( 1 == test )
		{
			movement = 1;
			print( px, vx, dx, yy++ );
			dx = movement * MoveAcceleration * elapsed;
...
void print( float px, float vx, float dx, unsigned char yy )
{
	engine_font_manager_draw_data( ( unsigned char ) px, 10, yy );
	engine_font_manager_draw_data( ( unsigned char ) vx, 20, yy );
	engine_font_manager_draw_data( ( unsigned char ) dx, 30, yy );
}
